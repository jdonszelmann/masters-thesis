...
use thiserror::Error;
use crate::grammar::grammar::{Captures, Pattern};
...
impl Display for ParsedPattern<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ParsedPattern::Match { regex, name, .. } => write!(f, "match {} to {}", regex.source, name.as_ref().map(|i| i.0.iter().join(".")).unwrap_or_else(|| "<unnamed>".to_string())),
            ParsedPattern::Surround { begin, end, name, .. } => write!(f, "surround {} .. {} to {}", begin.source, end.source, name.as_ref().map(|i| i.0.iter().join(".")).unwrap_or_else(|| "<unnamed>".to_string())),
            ParsedPattern::Any(v) => write!(f, "any of ({})", v.iter().map(|i| i.to_string()).join(", ")),
...
fn parse_include<'g>(include: &str) -> Result<ParsedPattern<'g>, ParsePatternError> {
    ...
    match chars.next() {
        Some('#') => {
            Ok(ParsedPattern::Include(chars.as_str().to_string()))
        }
...
fn parse_captures(captures: &Option<Captures>) -> Result<HashMap<usize, Capture>, ParsePatternError> {
    Ok(
        captures
        .as_ref()
        .map(|i| {
...
/// TODO: detect cycles
fn parse_pattern<'g>(pattern: &'g Pattern, cache: &mut Repository) -> Result<ParsedPattern<'g>, ParsePatternError> {
    Ok(match (&pattern.begin, &pattern.end, &pattern.match_) {
        ...
        (None, None, None) => {
            if let Some(i) = &pattern.include {
                ...
                let patterns = pattern.patterns.iter().map(|i| parse_pattern(i, cache)).collect::<Result<_, _>>()?;
        ...
        (Some(_), Some(_), Some(_)) => return Err(ParsePatternError::SurroundAndMatch(pattern.clone()))
    })
}
...
impl<'a, 'g> Parser<'a, 'g> {
    pub fn new(source: &'a str, grammar: &'g TextmateGrammar) -> Result<Self, ParseError> {
        ...
        for (name, pattern) in &grammar.grammar.repository {
            let pat =  parse_pattern(pattern, &mut repository)?;
    ...
    pub fn parse(&mut self) -> Result<Tokens, ParseError> {
        let patterns = self.grammar.grammar.patterns
            .iter()
            .map(|i| parse_pattern(i, &mut self.repository))
            .collect::<Result<Vec<_>, _>>()?;

        self.parse_with_patterns(patterns.as_slice())
    }

    fn try_pattern<'p>(&'g self, root_patterns: &'p [ParsedPattern<'g>], token: &str, start_offset: usize, i: &'p ParsedPattern<'g>, scope_stack: &ScopeStack<'p, 'g>) -> Result<Vec<TryPatternStatus<'p, 'g>>, ParseError>
        where 'a: 'p, 'g: 'p
    {
        match i {
            ParsedPattern::Match { regex, name, captures: grammar_captures} => {
                // TODO: captures
                if let Some(captures) = regex.regex.captures(token) {
                    ...
                    return Ok(vec![TryPatternStatus::Matched{start: entire_match.0, end: entire_match.1, tokens, pattern: i}])
                }
            }
            ParsedPattern::Surround {
                begin, end,
                ...
                if let Some(captures) = begin.regex.captures(token) {
                    ...
                    return Ok(vec![TryPatternStatus::MatchedStart{start: entire_match.0, end: entire_match.1, tokens, scope, pattern: i}])
                }

            }
            ParsedPattern::Any(patterns) => {

                return patterns.iter().map(|i| self.try_pattern(root_patterns, token, start_offset, i, scope_stack)).flatten_ok().collect()
            }
    ...
    fn try_end<'p>(&'g self, scope: Option<&Scope<'p, 'g>>, token: &str, start_offset: usize) -> TryPatternStatus<'p, 'g>
        where 'a: 'p, 'g: 'p
    {

        if let Some(Scope{end, original_pattern, start, end_captures, ..}) = scope {
            if let Some(captures) = end.regex.captures(token) {
                ...
                if let ParsedPattern::Surround { content_name: Some(name), .. } = original_pattern {
                    let span = Span {
    ...
    fn try_patterns<'p>(&'g self, patterns: &'p [ParsedPattern<'g>], token: &str, start_offset: usize, scope_stack: &ScopeStack<'p, 'g>) -> Result<Vec<TryPatternStatus<'p, 'g>>, ParseError>
        where 'a: 'p, 'g: 'p
    {
        let scope = scope_stack.last();
        let patterns = scope.map(|i| i.scope_patterns).unwrap_or(patterns);
    ...
    fn first_pattern_match<'p>(&'g self, root_patterns: &'p [ParsedPattern<'g>], token: &str, start_offset: usize, scope_stack: &ScopeStack<'p, 'g>) -> Result<TryPatternStatus<'p, 'g>, ParseError>
        where 'a: 'p, 'g: 'p {
    ...
    fn parse_line<'p>(&'g self, line: &str, span_offset: usize, root_patterns: &'p [ParsedPattern<'g>], tokens: &mut Tokens, scope_stack: &mut ScopeStack<'p, 'g>) -> Result<(), ParseError>
        where 'a: 'p, 'g: 'p
    {
        ...
        loop {
            ...
            match self.first_pattern_match(root_patterns, token, token_start + span_offset, scope_stack)? {
                TryPatternStatus::Matched{end, tokens: new_tokens, ..} => {
                    tokens.extend(new_tokens);
                    token_start += end;
                }
                TryPatternStatus::MatchedStart{end, tokens: new_tokens, scope, ..} => {
                    tokens.extend(new_tokens);
                    token_start += end;
                    scope_stack.push(scope);
                }
                TryPatternStatus::MatchedEnd{end, tokens: new_tokens, ..} => {
                    tokens.extend(new_tokens);
                    token_start += end;
                    scope_stack.pop();
                }
